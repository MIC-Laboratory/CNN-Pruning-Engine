...
block_channel_origin = [64,72,72,120,120,240,200,184,184,480,672,672,960,960]
block_channel_pruning = [64,72,72,120,120,240,200,184,184,480,672,672,960,960]
...
def Taylor_add_gradient():
    ...
    for block in range(len(block_channel_origin)):
        layer = new_net.features[block+2].block
...
def MobilenetV3Pruning(pruning_block):
    ...
    layer = new_net.features[pruning_block+2].block
    sorted_idx = get_sorted_idx(layer[1][0].weight.data.clone(),pruning_block)
    layer[0][0].weight.data = remove_filter_by_index(layer[0][0].weight.data.clone(),sorted_idx)
    layer[1][0].weight.data = remove_filter_by_index(layer[1][0].weight.data.clone(),sorted_idx)
    remove_Bn(layer[1][1],sorted_idx)
    remove_Bn(layer[0][1],sorted_idx)
    if isinstance(layer[2],torchvision.ops.misc.SqueezeExcitation):
        layer[2].fc1.weight.data = remove_kernel_by_index(layer[2].fc1.weight.data.clone(),sorted_idx)
        layer[2].fc1.in_channels -= len(sorted_idx)
        layer[2].fc2.weight.data,layer[2].fc2.bias.data = remove_filter_by_index(layer[2].fc2.weight.data.clone(),sorted_idx,bias=layer[2].fc2.bias.data.clone())
        layer[2].fc2.out_channels -= len(sorted_idx)
        layer[3][0].weight.data = remove_kernel_by_index(layer[3][0].weight.data.clone(),sorted_idx)
        layer[3][0].in_channels -= len(sorted_idx)
    else:
        layer[2][0].weight.data = remove_kernel_by_index(layer[2][0].weight.data.clone(),sorted_idx)
        layer[2][0].in_channels -= len(sorted_idx)
    layer[0][0].out_channels -= len(sorted_idx)
    layer[1][0].out_channels -= len(sorted_idx)
    layer[1][0].in_channels -= len(sorted_idx)
    layer[1][0].groups -= len(sorted_idx)
...
def bruth_force_calculate_k():
    global best_acc
    global K
    for layer in range(2,len(pruning_rate)):
        percentage = 0.1
        pruning_rate[layer] = 0 
        for _ in range(1):
            K=1
