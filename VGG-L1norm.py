...
from Vgg import LinView as lin
...
def VGG16Pruning():
    ...
    for old,new in zip(net.modules(),new_net.modules()):
        ...
        # real pruning
            ...
            if isinstance(old, nn.Conv2d):
                ...
                importance = torch.sum(torch.abs(old.weight.data), dim=(0, 2, 3))
    ...
    print("After Pruning: ")
    validation(0,network=new_net,file_name="VGG_L1norm.pth",save=False)
...
precentage = 0
writer = SummaryWriter("VGG-data/VGG-L1norm0")
...
#     writer = SummaryWriter("VGG-L1norm/layer"+str(idx))
#     precentage = 0
#     UpdateNet(idx, 1-precentage)
